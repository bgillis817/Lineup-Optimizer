...
load_pitches <- function() {
  ...
  dedup_pitches <- function(d) {
    ...
    else { kc <- intersect(c("GameID","GameUID","PitchNo","Pitcher","Batter","Inning","PAofInning","PitchofPA"), names(d))
      if (length(kc) >= 3) d <- d %>% distinct(across(all_of(kc)), .keep_all = TRUE) }
    if (nrow(d) < before) message("de-duplicated pitches: ", before, " -> ", nrow(d))
    d
  }
  if (cache != "" && file.exists(cache) && Sys.getenv("REFRESH","0") != "1") {
    message("pitch source: cache ", cache); return(dedup_pitches(readRDS(cache)))
  }
  ...
  # --- de-duplicate: duplicate game CSVs in the folder double-count pitches ---
  before <- nrow(d)
  if ("PitchUID" %in% names(d)) {
    d <- d %>% distinct(PitchUID, .keep_all = TRUE)
  } else {
    key_cols <- intersect(c("GameID","GameUID","PitchNo","Pitcher","Batter","Inning","PAofInning","PitchofPA"), names(d))
    if (length(key_cols) >= 3) d <- d %>% distinct(across(all_of(key_cols)), .keep_all = TRUE)
  }
  if (nrow(d) < before) message("de-duplicated pitches: ", before, " -> ", nrow(d),
                                " (removed ", before - nrow(d), " duplicate rows)")
...
# ---- attach xwOBACON to every batted ball (shared by pitcher & batter shape) -
# adds px$xwobacon AND the 5 outcome probs (xp_out, xp_1b, xp_2b, xp_3b, xp_hr)
# so downstream code can build a real contact distribution, not just a scalar.
attach_xwobacon <- function(px) {
  xw <- tryCatch(load_xwobacon(), error = function(e) { message("xwOBACON model absent: xwobacon = NA"); NULL })
  px$xwobacon <- NA_real_
  for (nm in c("xp_out","xp_1b","xp_2b","xp_3b","xp_hr")) px[[nm]] <- NA_real_
  if (is.null(xw)) return(px)
  bip <- which(px$IsBIP & !is.na(px$ExitSpeed) & !is.na(px$Angle))
  if (!length(bip)) return(px)
  pr <- xw(px$ExitSpeed[bip], px$Angle[bip], if ("Bearing" %in% names(px)) px$Bearing[bip] else 0)
  px$xwobacon[bip] <- as.numeric(pr %*% WOBA_VEC)
  px$xp_out[bip] <- pr[,1]; px$xp_1b[bip] <- pr[,2]
  px$xp_2b[bip]  <- pr[,3]; px$xp_3b[bip] <- pr[,4]; px$xp_hr[bip] <- pr[,5]
  px
}
